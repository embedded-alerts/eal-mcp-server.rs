...
fn official_rmcp_process_preserves_protocol_and_tool_contract() {
    ...
    assert!(
        !String::from_utf8_lossy(responses.get("10").unwrap()).contains("embedded-alerts/unknown")
    );
    ...
    assert!(
        !String::from_utf8_lossy(responses.get("7").unwrap()).contains("embedded-alerts/unknown")
    );
}
