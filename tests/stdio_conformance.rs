...
use eal_mcp_server::{SERVER_NAME, dependency_graph};
use ore_mcp_testkit::{
...
fn initialize_request(id: u64, protocol_version: &str) -> Value {
    json!({
        ...
        "params": {
            ...
            "capabilities": {},
            "clientInfo": {"name": "den-3035-test", "version": "1.0.0"}
        }
    })
}

fn run_session_with_status(requests: &[Value]) -> (ExitStatus, Vec<u8>, Vec<u8>) {
    let mut child = Command::new(env!("CARGO_BIN_EXE_eal-mcp-server"))
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn Embedded Alerts MCP server");
...
fn official_rmcp_process_preserves_protocol_and_tool_contract() {
    let requests = [
        ...
        json!({
            "jsonrpc": "2.0",
            "id": 7,
            "method": "embedded-alerts/unknown",
            "params": {}
        }),
        json!({
            "jsonrpc": "2.0",
        ...
        json!({
            ...
            "id": 10,
            "method": "tools/call",
            "params": {"name": "embedded-alerts/unknown", "arguments": {}}
        }),
    ];
    ...
    assert!(!String::from_utf8_lossy(responses.get("10").unwrap()).contains("embedded-alerts/unknown"));
    ...
    assert!(!String::from_utf8_lossy(responses.get("7").unwrap()).contains("embedded-alerts/unknown"));
}
