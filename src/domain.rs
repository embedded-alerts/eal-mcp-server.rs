...
pub const REPOSITORY: &str = "eal-mcp-server.rs";
pub const DOMAIN_SUMMARY: &str = "Read-only MCP diagnostics for embedding rules, events, delivery, and acknowledgement contracts";
pub const UNIT_LABEL: &str = "alert units";

const REPOSITORIES: [(&str, &str); 5] = [
    ("eal-api", "rule and delivery-state API"),
    (
        "eal-interfaces",
        "embedding, rule, event, and transport contracts",
    ),
    (
        "embedded-alerts-libs",
        "provider, evaluation, delivery, and policy libraries",
    ),
...
impl Workload {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::DeliveryFanout => "delivery_fanout",
            Self::AcknowledgementSync => "acknowledgement_sync",
        }
    }
}
...
pub struct PlanInput {
    ...
    /// Bounded number of alert units to evaluate.
...
pub const fn workload_names() -> [&'static str; 3] {
    ["rule_evaluation", "delivery_fanout", "acknowledgement_sync"]
}
