//! Embedded Alerts' read-only MCP server contract.
//!
...
pub const SERVER_TITLE: &str = "Embedded Alerts MCP Server";
...
const ORGANIZATION: &str = "embedded-alerts";
const REPOSITORY: &str = "embedded-alerts/eal-mcp-server.rs";
const PACKAGE: &str = "eal-mcp-server";
...
/// Stateless, read-only Embedded Alerts MCP handler.
...
pub fn dependency_graph() -> DependencyGraph {
    DependencyGraph::new(ORGANIZATION, REPOSITORY, PACKAGE, DEPENDENCIES)
        .expect("static Embedded Alerts dependency graph must remain valid")
}
