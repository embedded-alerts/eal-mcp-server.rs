...
pub const SERVER_NAME: &str = "eal-mcp-server";
pub const SERVER_NAMESPACE: &str = "embedded-alerts";

#[derive(Clone)]
pub struct EmbeddedAlertsMCPServer {
    tool_router: ToolRouter<Self>,
    metrics: ToolMetrics,
    lifecycle: Arc<LifecycleController>,
}

impl EmbeddedAlertsMCPServer {
    #[must_use]
...
impl EmbeddedAlertsMCPServer {
    ...
    #[tracing::instrument(name = "mcp.tool", skip_all, fields(mcp.tool.name = "eal_fleet_map", mcp.tool.class = "inventory"))]
    fn eal_fleet_map(&self) -> String {
    ...
    #[tracing::instrument(name = "mcp.tool", skip_all, fields(mcp.tool.name = "eal_plan", mcp.tool.class = "details"))]
    fn eal_plan(&self, Parameters(input): Parameters<PlanInput>) -> Result<String, String> {
    ...
    #[tracing::instrument(name = "mcp.tool", skip_all, fields(mcp.tool.name = "eal_runtime_readiness", mcp.tool.class = "health"))]
    fn eal_runtime_readiness(&self) -> String {
    ...
    #[tracing::instrument(name = "mcp.tool", skip_all, fields(mcp.tool.name = "eal_shared_platform", mcp.tool.class = "inventory"))]
    fn eal_shared_platform(&self) -> String {
        let timer = self.metrics.start(ToolClass::Inventory);
    ...
    #[tracing::instrument(name = "mcp.tool", skip_all, fields(mcp.tool.name = "eal_lifecycle_state", mcp.tool.class = "health"))]
    fn eal_lifecycle_state(&self) -> Result<String, String> {
        let timer = self.metrics.start(ToolClass::Health);
        let result = self
            .lifecycle
            .snapshot_and_audit()
    ...
    #[tracing::instrument(name = "mcp.tool", skip_all, fields(mcp.tool.name = "eal_safety_boundary", mcp.tool.class = "inventory"))]
    fn eal_safety_boundary(&self) -> String {
        let timer = self.metrics.start(ToolClass::Inventory);
...
impl ServerHandler for EmbeddedAlertsMCPServer {
    ...
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            .with_server_info(Implementation::new(SERVER_NAME, env!("CARGO_PKG_VERSION")).with_title("Embedded Alerts MCP Server"))
            .with_instructions("Read-only MCP diagnostics for embedding rules, events, delivery, and acknowledgement contracts. The server is read-only and never logs MCP arguments or results.")
    }
}
...
mod tests {
    ...
    fn exact_prefixed_tool_catalog_is_exposed() {
        let server =
            EmbeddedAlertsMCPServer::new(LifecycleController::new(8).expect("valid lifecycle"));
        let names = server
            .tool_router
            .list_all()
            .into_iter()
