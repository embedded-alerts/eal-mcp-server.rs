...
#
# Multi-stage image for eal-mcp-server.
# Prefer linux/arm64:
#   docker buildx build --platform linux/arm64 -t eal-mcp-server:dev .
#   docker run --rm --platform linux/arm64 \
#     -e SOPS_AGE_KEY="$(cat ~/.config/sops/age/keys.txt)" eal-mcp-server:dev
#
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry,sharing=locked \
    ...
    --mount=type=cache,target=/src/target,id=eal-mcp-server-target-${TARGETARCH},sharing=locked \
    cargo build --release --locked --bin eal-mcp-server \
    && strip "target/release/eal-mcp-server" \
    && cp "target/release/eal-mcp-server" "/usr/local/bin/eal-mcp-server"
...
COPY --from=build "/usr/local/bin/eal-mcp-server" "/usr/local/bin/eal-mcp-server"
...
RUN --mount=type=bind,source=.,target=/src,ro \
    ...
    && chown -R 65532:65532 /app /usr/local/bin/eal-mcp-server
ENV SOPS_SECRETS_FILE=/app/secrets/app.env \
    OTEL_SERVICE_NAME=eal-mcp-server \
    OTEL_EXPORTER_OTLP_ENDPOINT=http://dd-otel-collector.observability.svc.cluster.local:4317 \
    RUST_LOG=info
USER 65532:65532
EXPOSE 8080
ENTRYPOINT ["/usr/local/bin/sops-entrypoint.sh", "/usr/local/bin/eal-mcp-server"]
